...
\usepackage{geometry} %permet de régler facilement les marges
...
@
    \caption [Predictiv intervals of the different models.]
    {\textbf{Predictiv intervals of the different models.} 
        Here are presented for each model and each species the relation between 
        the three 
        characteristic functions at the tree scale and tree diameter. 
        $F(D)$ is the appearence probability of at least one tree microhabitat 
        before the tree reach a diameter of $D$;
        $h(D)$ is the appearence probability of at least one tree microhabitat 
        during a $dD$ diameter increment;
        $f(D)$ is the appearence probability of the first tree microhabitat 
        during a $dD$ diameter incement. 
        The gray buffers depict the $95\%$ predictiv interval, the solid line
        represent the median prediction.}
    \label{fig:Comp}
\end{figure}
...
@
    ...
    {\textbf{Frequences of tree bearing microhabitat} by diameter
        ...
        the gray dashed lines figure the $95\%$ predictiv intervals.
      \label{fig:IP}}
\end{figure}
...
@
    ...
    {\textbf{Frequencies of tree bearing $n$ microhabitats at the stand 
        ...
        the dashed lines figure the $95\%$ predictiv intervals. 
    \label{fig:Ndmh_l}}
\end{figure}
...
@
    \caption [Evolution of the probability to bear $n$ microhabitat at 
        the tree scale.]
    {\textbf{Evolution of the probability to bear $n$ microhabitat at 
        ...
        simulated each year.
        The solid lines represent the median values of predictions, the dashed 
        lines depict the $95\%$ predictiv intervals. 
    \label{fig:Ndmh_tree}}
\end{figure}
...
table[1:3]<- apply(table[1:3], 
                MARGIN = 2, 
                FUN = as.numeric(x))
latex(as.tabular(table), digits=2)
@
  \caption []
        {\textbf{}
        \label{}} 
...
dat <- expand.grid(Species=c("Abies a.", "Fagus s."),
                   Parameters=c("$\\lambda$", "$k$"),
                   Models=c("Weibull", "Rayleigh", "Exponential"))
...
@
	\caption []
        {\textbf{Results of the MCMC per model and species.}
